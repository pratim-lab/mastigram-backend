...
class EmailInitiateOrderAdmin extends Mailable
{
   ...
   public function __construct($user,$order_data,$main_order_data)
    ...
    public function build(){
		...
		return $this->from(config('global.admin_email_id'))
                    ->subject(config('global.website_title_camel_case').'—'.'A new order "'.$this->main_order_data->unique_order_id.'" has been initiated')
                    ->markdown('site.emails.order.initiated_order_admin')->with(['data'=>$this->user,'order_data'=>$this->order_data,'main_order_data'=>$this->main_order_data]);
    }
}